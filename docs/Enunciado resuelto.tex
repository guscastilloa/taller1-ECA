...
Ahora cada investigador $i$ debe escoger $r\in\mathcal{S_i}$ . Aún a pesar de que todo $r\in \mathcal{S_i}$ será nuevo para $i$, la probabilidad de que $i$ escoja un \textit{trending research topic} se podría representar como

\begin{equation}
    \label{eq:alt2_prob}
    f(Y^1_i,Y^0_i,,\mathbf{X}_i,u_i)
\end{equation}

donde $f$ es una función de las características observables y no observables de $i$. 
...
donde $f(\epsilon_i)$ es una función que depende de las características no observables del investigador.
    ...
    \begin{itemize}
      ...
      Cabe aclarar que tenemos dos supuestos importantes detrás. Primero, que existe más de un \textit{trending reserach topic} en la base de datos y segundo que para cada investigador $i$ se considera que existe al menos un $r\in \mathcal{S_i}$. Con esto en mente, entonces cada investigador va a tener una probabilidad mayor que 0 de que se le asigne un trending research topic.

      Como además nos indican que para cada investigador todos los temas tienen la misma probabilidad de ser asignados, y suponiendo que $|R|$ es grande, lo suficiente como para que $|\mathcal{S_i}|$ siga siéndolo, entonces esta probabilidad sigue una distribución uniforme y por lo tanto es aleatoria, y además no está relacionada con los resultados potenciales de $i$. 
...
\begin{solucion}
        ...
        Si la asignación no es aleatoria, el principal problema que se podría generar es que no podríamos recuperar el efecto causal de interés de publicar en un \textit{trending research topic} a causa del sesgo de selección. Si consideramos la \textbf{Alternativa 2} el mecanismo no es ignorable y es probable que aquellos que terminaron seleccionando \textit{trending research topics} (se autoseleccionan al tratamiento) y aquellos que no (se autoseleccionan al control) serán \textbf{sistemáticamente diferentes}. Así, no podríamos tener contrafactuales válidos. La clave radica en que si no hay sesgo selección, podemos estimar el $ATT$ con la comparación ingenua que es $ \tau = E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0]$.  
        \end{solucion}
    \end{itemize}
...
\end{proof}
    ...
    \begin{itemize}
        \item[a.] $\tau = \text{ATT}+\left( \mathbb{E}[Y^{0}_{i}|D_i=1]-\mathbb{E}[Y^{0}_{i}|D_i=0] \right)$
          ...
          \begin{solucion}
         \begin{proof}
        Definimos  $ \tau = E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0]$. Sabemos que  ATT =$E[Y_i^1 - Y_i^0 | D_i=1]$. Entonces:
        \begin{align*}
         \tau &= E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0]\\
         &= E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0] \quad \text{(Sumando y restando $E[Y_i^0| D_i=1]$ )} \\ &+E[Y_i^0| D_i=1] - E[Y_i^0| D_i=1]  \\
       &=  E[Y_i^1| D_i=1]-E[Y_i^0| D_i=1] +E[Y_i^0| D_i=1]- E[Y_i^0| D_i=0]  \quad \text{(reorganizando)} \\
     &=  \text{ATT}+\left( \mathbb{E}[Y^{0}_{i}|D_i=1]-\mathbb{E}[Y^{0}_{i}|D_i=0] \right)\\
     \end{align*}
\end{proof}
\begin{mdframed}[backgroundcolor=moraditoClaro]
\textbf{Caveat:} Note que usando el supuesto de que el experimento es aleatorio, del literal 4, tenemos entonces $\tau$ es igual al $\tau_{ingenua}$ ya que el sesgo de selección es 0. Trivialmente con nuestro supuesto de efectos homogéneos $\tau = E[Y_i^1] - E[Y_i^0]= ATT$ pues $ \mathbb{E}[Y^{0}_{i}|D_i=0] = \mathbb{E}[Y^{0}_{i}|D_i=1]= \mathbb{E}[Y^{0}_{i}] $ y  $ \mathbb{E}[Y^{1}_{i}|D_i=0] = \mathbb{E}[Y^{1}_{i}|D_i=1]= \mathbb{E}[Y^{1}_{i}] $ 
 ...
 \end{mdframed} 
        ...
        \item[b.]  $\tau= \text{ATU}+\left( \mathbb{E}[Y^{1}_{i}|D_i=1]-\mathbb{E}[Y^{1}_{i}|D_i=0] \right)$
         \begin{solucion}
         \begin{proof}
        Definimos  $ \tau = E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0]$. Sabemos que  ATU =$E[Y_i^1 - Y_i^0 | D_i=0]$. Entonces:
        \begin{align*}
         \tau &= E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0]\\
         &= E[Y_i^1| D_i=1] - E[Y_i^0| D_i=0] \quad \text{(Sumando y restando $E[Y_i^1| D_i=0]$ )} \\ &+E[Y_i^1| D_i=0] - E[Y_i^1| D_i=0]  \\
       &=  E[Y_i^1| D_i=0]-E[Y_i^0| D_i=0] +E[Y_i^1| D_i=1]- E[Y_i^1| D_i=0]  \quad \text{(reorganizando)} \\
     &=  \text{ATU}+\left( E[Y_i^1| D_i=1]- E[Y_i^1| D_i=0]  \right)\\
     \end{align*}
\end{proof}
\begin{mdframed}[backgroundcolor=moraditoClaro]
\textbf{Caveat:}Note que usando el supuesto de que el experimento es aleatorio, del literal 4, tenemos entonces $\tau$ es igual al $\tau_{ingenua}$ ya que el sesgo de selección es 0. Trivialmente con nuestro supuesto de efectos homogéneos $\tau = E[Y_i^1] - E[Y_i^0]= ATT$ pues $ \mathbb{E}[Y^{0}_{i}|D_i=0] = \mathbb{E}[Y^{0}_{i}|D_i=1]= \mathbb{E}[Y^{0}_{i}] $ y  $ \mathbb{E}[Y^{1}_{i}|D_i=0] = \mathbb{E}[Y^{1}_{i}|D_i=1]= \mathbb{E}[Y^{1}_{i}] $ 
...
\begin{solucion}
    ...
    \begin{itemize}
       ...
       Sea C una variable dummy tal que:
        \[
C_i =
\begin{cases} 
...
\end{gather}
Después de obtener $\theta'$, que es el coeficiente de la regresión. Se debe usar la ecuación \ref{eq:finding_theta} que derivamos en el literal anterior. Entonces $\theta= \theta'E[C|D=1]$, donde el último termino lo obtenemos hacer un promedio de los investigadores que colaboraron dado que escribieron sobre trending reserach topics. 
\end{solucion}
...
\begin{solucion}
        ...
        b.) Teóricamente, esta propuesta será exitosa porque permite aislar el efecto de solamente publicar en un trending research topic, cuando el proyecto no es colaborativo. Al incluir $C_i$ como control ``sacamos'' dicho efecto del error, resolviendo la variable omitida.Estos estimadores van a ser consistentes por la demostración del punto 8.\\
        
        Empíricamente, como mencionamos el sesgo de interés $\theta$ va a ser $\theta'E[C|D=1]$, el cual es coherente con los resultados obtenidos en las estimaciones usando la base de datos.\\
    
\end{solucion}
        \item[c.] Estime la regresión $Y_i = \alpha + \tau D_i + u_i$.
\begin{solucion}
Los resultados se encuentran en el literal d
\end{solucion}
...
\begin{solucion}
        Claim: $\mathbf{\hat{y}} = \mathbf{P}\mathbf{y}$ 

Veamos que $\boldsymbol{\hat{y}} = \boldsymbol{X}\boldsymbol{\hat{\beta}}$ es la proyección de $\boldsymbol{y}$ sobre el espacio generado por $\boldsymbol{X}$. Sabemos $\hat{\beta} =(X'X)^{-1} X' y.$ \quad Luego: 
\begin{proof}
\begin{align*}
    \hat{y} &= X \hat{\beta} \\
    \hat{y} &= X (X'X)^{-1} X' y  \\
    &= Py \quad \text{(pues $P=\mathbf{X}(\mathbf{X'}\mathbf{X})^{-1}\mathbf{X'}$)}
\end{align*}
\end{proof}
...
\begin{solucion}
  Claim: \(\hat{\epsilon} = \mathbf{M} \mathbf{y}\)
  \begin{proof} Sabemos que  $\hat{\epsilon} = y - \hat{y}$ y que $
    y= X\beta + \epsilon$. Luego:
  \begin{align*}
    \hat{\epsilon} &=y - \hat{y} \\
    &= X\beta + \epsilon - X\hat{\beta}_{\text{mco}} \\
    &= X\beta + \epsilon - X (X'X)^{-1} X' y \\
    &= X\beta + \epsilon - X (X'X)^{-1} X' (X\beta + \epsilon) \\
    &= X\beta + \epsilon - X (X'X)^{-1} X' X \beta - X (X'X)^{-1} X' \epsilon \\
    &= (I_n - X (X'X)^{-1} X') (X\beta + \epsilon)\\
     &= My
     \end{align*}
    \end{proof}
\end{solucion}
...
\textbf{Ampliando la expresión anterior}
...
De donde derivamos el siguiente sistema:
...
\begin{mdframed}[backgroundcolor=moraditoClaro]
\textbf{Caveat:} Note que de manera alterna, usted puede llegar a estas ecuaciones, haciendo la minimización del error. Esto es: 
\begin{align*}
    S(\hat{\beta})&= (y - X_1 \hat{\beta}_1 - X_2 \hat{\beta}_2)'(y - X_1 \hat{\beta}_1 - X_2 \hat{\beta}_2) \\
    &= y'y - y'X_1 \hat{\beta}_1 - y'X_2 \hat{\beta}_2 - \hat{\beta}_1'X_1' y + \hat{\beta}_1'X_1'X_1\hat{\beta}_1 + X_2' \hat{\beta}_2 X_2 \hat{\beta}_2 \\
    &\quad - X_2' \hat{\beta}_2 y + X_2' \hat{\beta}_2 X_1 \hat{\beta}_1 + X_2' \hat{\beta}_2 X_2 \hat{\beta}_2
\end{align*}

Derivando respecto a \( \beta_1 \):

\begin{align*}
    \frac{\partial S(\hat{\beta})}{\partial \beta_1} &= - 2 X_1' y + 2 X_1' X_1 \hat{\beta}_1 + 2 X_1' X_2 \hat{\beta}_2 = 0 \\
    &\Rightarrow X_1' y = X_1' X_1 \hat{\beta}_1 + X_1' X_2 \hat{\beta}_2
\end{align*}
Derivando respecto a \( \beta_2 \):

\begin{align*}
    \frac{\partial S(\hat{\beta})}{\partial \beta_2} &= - 2 X_2' y + 2 X_2' X_1 \hat{\beta}_1 + 2 X_2' X_2 \hat{\beta}_2 = 0 \\
    &\Rightarrow X_2' y = X_2' X_1 \hat{\beta}_1 + X_2' X_2 \hat{\beta}_2
\end{align*}
Armando el sistema de ecuaciones de forma matricial, esto es: 
\[
\begin{bmatrix}
X_1^\top X_1 & X_1^\top X_2 \\
X_2^\top X_1 & X_2^\top X_2
...
%██████  ██████  
    ...
    \item Demuestre que la matriz $\textbf{P}_1 \equiv \textbf{X}_1(\textbf{X}_1'\textbf{X}_1)^{-1}\textbf{X}_1'$ es una matriz de proyección ortogonal y defina la matriz de aniquilación $\textbf{M}_1$ asociada a $\textbf{X}_1$. Demuestre que $\textbf{M}_1$ también es una matriz de proyección ortogonal.

\begin{solucion}

 I Claim: $P_1 = X_1(X_1'X_1)^{-1}X_1'$ es una proyección ortogonal. Esto es, P es idempotente y simétrica.\\
\textbf{Idempotencia:}
\begin{proof}
\begin{align*}
...
II. Claim:  la matriz de aniquilación $M_1 = I - P_1$ es una proyección ortogonal. Esto es, es idempotente y simétrica.
...
\end{solucion}
 Así mismo, demuestre que 
    \begin{enumerate}\label{aniquiladoranotaciona}
        ...
        \begin{solucion}
 Claim: $M_1$ aniquila a $X_1$. Para ello, debemos demostrar que $M_1X_1 = \boldsymbol{0}$.
\begin{proof}
\begin{align*}
    M_1X_1 &= (I - P_1)X_1 \\
    &= X_1 - P_1X_1 \\
...
\end{solucion}
        ...
        \begin{solucion}
 Claim:  $M_1\epsilon = \epsilon$. Esta propiedad nos dice que la matriz de aniquilación $M_1$ no afecta a $\epsilon$, pues este es aleatorio y no debería contener información de las $X$
...
III. Claim: Sea \(\tilde{\beta} = \hat{\beta} + \mathbf{C}\mathbf{y}\) entonces
$
\mathrm{Var}(\tilde{\beta}) = \sigma^2 (\mathbf{X}'\mathbf{X})^{-1} + \sigma^2\,\mathbf{C}\mathbf{C}'.
$ 
   \begin{proof}
   
   Como sabemos, el estimador de MCO es $\hat{\beta}=(X'X)^{-1}X'y$. Ahora, sea $ \tilde{\beta} =Ay$.Luego:
   \begin{align}
       \tilde{\beta} &=Ay \notag \\
   ...
   Como $\beta$ es una constante, no contribuye a la varianza y podemos ignorarlo para calcular $\mathbb{V}(\tilde{\beta})$. Por el mismo motivo, como $Cov(\beta, [(X'X)^{-1}X'+C]\epsilon)$ también es 0. Así:
...
var_i = \beta_0 + \beta_{incentives} D_{i1} + \beta_{automnomy} D_{i2} +  \beta_{both} D_{i3} + \varepsilon_i$, los coeficientes asociados a cada estado del tratamiento sean estadísticamentes iguales a 0.Luego cuando sacamos los valores esperados para cada grupo de tratamiento i.e. $E[var \mid D_{i1}=1], E[var \mid D_{i2}=1],E[var \mid D_{i3}=1], E[var \mid D_{i1}=0, D_{i2}=0, D_{i3}=0]$, queremos que estos sean estadísticamente iguales, que es lo que hacemos con una prueba de diferencia de medias. Ahondaremos sobre esto en c).
        ...
        \item En la práctica, ¿una asignación aleatoria al tratamiento garantiza inequívocamente el cumplimiento de correcto balance muestral? (Menos de 100 palabras)
          \begin{solucion}
  No  necesarimente. Esto porque siempre hay una posibilidad de rechazar la hipótesis nula aún cuando sea verdadera. Este es el error tipo 1 (el valor de $\alpha$). Este es usualmente $1\%, 5\% \quad o \quad 10\%$ entonces siempre existirá algo de incertidumbre.
        \end{solucion}
...
%         \end{proof}
%         Ahora, como empíricamente obtuvimos $\hat{\tau}^*_{MCO} \approx 5.16$ y teóricamente $\tau \approx 4$ entonces el sesgo  $\theta \approx 1.16$.  Esto es congruente con los resultados obtenidos en cuanto el estimador empírico es mayor que el estimador teórico.
%         \end{solucion}

 (resaltamos que no es que sea de inferencia per ser sino que esto ocapuede derivar un problema de sesgo de selección, que sí es un problema de inferencia causal).. El sesgo de selección es un indicativo de que el error y el Adicionalmente, tener un con
       . En particular, las alternativas 1 y 2 sugieren que el efecto va a estar subestimado, pues varios investigadores asignados al grupo de control, podrían ya ser populares por sus trabajos previos en los trendings reserach topics.
