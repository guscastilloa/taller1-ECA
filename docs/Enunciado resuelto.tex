...
En este procedimiento omitiremos las negrillas para simplificar la notación, pero a menos de que especifique lo contrario, las mayúsculas hacen referencia a matrices y las minúsculas a vectores.
\begin{proof}
    
    Supongamos que $\tilde{\beta}$ es insesgado. Entonces
   ...
   Como $\beta$ es una constante no contribuye a la varianza y podemos ignorarlo para calcular $\mathbb{V}(\tilde{\beta})$:
   
   \begin{align}
   \mathbb{V}(\tilde{\beta}) &= \mathbb{V}([(X'X)^{-1}X'+C]\epsilon) \qquad (A=(X'X)^{-1}X'+C) \notag \\ 
    &= \mathbb{V}(A\epsilon) \qquad (\text{varianza de una matriz constante}) \notag \\
    &= A\mathbb{V}(\epsilon)A' \qquad (\mathbb{V}(\epsilon)=\sigma^2I_n) \notag \\
    &= \sigma^2 AA' \qquad (\text{definamos } X^\dagger=(X'X)^{-1}X' ) \notag \\
    &= \sigma^2(X^\dagger+C )(X^\dagger+C)' \notag \\
   ...
   \begin{align}
       \mathbb{V}(\tilde{\beta}) &= \sigma^2(X'X)^{-1} + \sigma^2CC'
   \end{align}
   como queríamos.
\end{proof}
