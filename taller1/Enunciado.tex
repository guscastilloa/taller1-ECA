...
\begin{align*}
    Y_i &= \alpha + u_i
\end{align*}
Entonces la diferencia de medias condicional entre tratados y no tratados es:
\begin{align*}
    E[Y_i | D_i = 1] - E[Y_i | D_i = 0] &= \alpha + \tau + E[u_i | D_i = 1]-\alpha -E[u_i | D_i = 0] \\ &= \tau + E[u_i | D_i = 1] -E[u_i | D_i = 0]
\end{align*}

Como estamos en el mundo de estimación por MCO, se supone exogeneidad, esto es que $E[u_i | D_i ]=0$. Por tanto no hay sesgo de selección y $ E[u_i | D_i = 1] - E[u_i | D_i = 0]=0$. \textcolor{red}{esto se puede suponer?}. Entonces, la diferencia de medias condicional identifica 
efecto causal $\tau$:

\begin{equation*}
    E[Y_i | D_i = 1] - E[Y_i | D_i = 0]=\tau
\end{equation*}

Usando la muestra, tenemos que:
\begin{itemize}
    \item $\hat\tau_{\scriptsize{MCO}}$ es el estimador por MCO de $\tau$ usando \ref{estmco}   
    \item $\frac{1}{N_1}\sum_{{i : D_i = 1}} Y_i$ es el análogo muestral de $ E[Y_i | D_i = 1]$
    \item $\frac{1}{N_0}\sum_{{i : D_i = 0}} Y_i$ es el análogo muestral de $E[Y_i | D_i = 0]$
\end{itemize}
Entonces:
 \begin{align*} \hat\tau_{\scriptsize{MCO}} &= \frac{1}{N_1}\sum_{{i : D_i = 1}} Y_i - \frac{1}{N_0}\sum_{{i : D_i = 0}} Y_i \\ &= \hat\tau_{\scriptsize{DM}} \end{align*}
\end{proof}
...
\begin{itemize}
    ...
    \centering
    \includegraphics[width=0.8\textwidth]{taller1/resultados/graficoconinter.png} 
